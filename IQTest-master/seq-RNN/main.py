...
def main():
    ...
    for t in range(n_iters + 1):
        ...
        if t % print_every == 0:
            guess=output
            correct = '✓' if guess == real_value else '✗'
        ...
        if t % plot_every == 0 and t!=0:
            ...
            data=seq_valid_data.__getitem__()
            real_value=data[-1][-1][2]
    ...
    img_path='img/train_eval'+str(n_iters/10000)+'w_'+'lr'+str(args.lr)+'_hidden'+str(args.hidden_size)+'_norm'+str(int(args.norm))+'.png'
    if args.save==1:
    ...
    model_path='model/RNNmodel'+str(n_iters/10000)+'w_'+'lr'+str(args.lr)+'_hidden'+str(args.hidden_size)+'_norm'+str(args.norm)+'.pkl'
    if args.save==1:

...
data_file={'question':'seq-public.json','answer':'seq-public.answer.json','private':'seq-private.json',\
        'train':'seq-train.json','valid':'seq-valid.json','test':'seq-test.json'}
...
if __name__ == "__main__":
    ...
    private_file=os.path.join(args.data_dir, data_file['private'])
    seqpri=open(private_file, 'r',encoding='utf-8')
    seqpri = json.load(seqpri)
    print(seqpri)
    test_dict={}
    for seq in seqpri:
        test_dict[seq['id']]={}
    ...
    test_file=open(os.path.join(args.data_dir, data_file['test']), "w", encoding='utf-8')
